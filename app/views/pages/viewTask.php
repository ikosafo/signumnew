                                ...
                                <form class="row" id="needs-validation1" novalidate="" autocomplete="off">
                                    ...
                                    <div class="next-btn py-4 d-flex col-sm-12 justify-content-center">
                                        ...
                                        if ($complaintDetails['resolution'] != "" && $complaintDetails['resolution'] != "Pending") {
                                            ...
                                            echo '<button type="button" id="updateResolution" class="updateResolution btn btn-success next2 btn-sm" i_index="' . $complaintDetails['complaintid'] . '" style="margin-left:10px">Update Resolution</button>';
                                        }
                                        ?>
...
<script>
    ...
    navItems.forEach(function(item) {
        ...
        if (textContent === 'MAINTENANCE') {
            console.log("Found MAINTENANCE:", item); 
            item.closest('li').classList.add('mm-active');
        }
    });

    $('#backList').click(function() {
        window.location.href = urlroot + '/pages/maintenanceTasks'
    });
    ...
    $(document).on('click', '.updateResolution', function() {
        var idIndex = $(this).attr('i_index');
        //alert(idIndex);
        ...
        var url = "/forms/updateResolution";
        var successCallback = function(response) {
            $('#verifyIssue').html(response);
        };
        saveForm(formData, url, successCallback);
