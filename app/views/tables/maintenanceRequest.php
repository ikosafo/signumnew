                    ...
                    <tbody>
                        ...
                        foreach ($listClients as $result) { ?>
                            <tr>
                                ...
                                <td>
                                    <div class="d-flex">
                                        <a href="javascript:void(0);" class="btn btn-success logRequest" clientid='<?= $result->clientid ?>'>Log Request</a>
                                    </div>
                                </td>
                            </tr>
                        <?php } ?>
...
<script>
    ...
    $(document).on('click', '.logRequest', function() {
        var clientid = $(this).attr('clientid');
        $('html, body').animate({
            scrollTop: $("#logRequestDiv").offset().top
        }, 500);

        var formData = {};
        formData.clientid = clientid; 
        saveForm(formData, "/forms/maintenanceRequest", function(response) {
            $('#logRequestDiv').html(response);
        });
    });
